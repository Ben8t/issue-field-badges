...
- **Settings and pins.** Your list of field names, the age badge, pull request badge, kanban, sidebar and pinned issues preferences (including the field the board groups by, the one it orders by, and whether the issue sidebar is folded away), and the issues you pin (repository, number, title, type) are stored with `chrome.storage.local` on your device.
