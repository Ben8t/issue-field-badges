...
**Epic kanban**: a List and Kanban toggle above the sub-issues list turns an epic into a board; you pick the field that forms the columns and the field that orders the cards inside each column.
...
- An **epic kanban**: above the sub-issues list of an issue, a List and Kanban toggle draws those sub-issues as a board. The columns come from any field the sub-issues carry (or their state, type or assignee), the cards inside a column are ordered by another, and both choices are remembered. Reading only: cards cannot be dragged. Can be switched off in the settings.
...
- The board only reads. To move an issue, change its field on the issue itself; the board follows once the five-minute cache expires or the page is reloaded.
...
- The kanban has its own path: `background.js` reads the open issue's `subIssues` connection, paginated by 100 up to 500 sub-issues, returning every field value each one carries plus its state, type, assignees and creation date. `content.js` groups and orders those in the page and draws the board in place of the list, so changing the column or the sort field redraws without another request. The result is cached per epic like everything else.
